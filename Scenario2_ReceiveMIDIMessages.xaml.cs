...
namespace MIDI
{
    ...
    public sealed partial class Scenario2_ReceiveMIDIMessages : Page
    {
        ...
        private async void MidiInputDevice_MessageReceived(MidiInPort sender, MidiMessageReceivedEventArgs args)
        {
            ...
            //Check RawData ÅüÅü
            var rawDataReader = DataReader.FromBuffer(receivedMidiMessage.RawData);
            while (rawDataReader.UnconsumedBufferLength > 0)
            {
                byte byteRead = rawDataReader.ReadByte();
                // Pad with leading zero if necessary
                //outputMessage.Append(byteRead.ToString("X2")).Append(" ");
                Debug.Write(byteRead.ToString("X2") + " ");
            }
            Debug.WriteLine("");
